    ...
    if [ -f "$pid_file" ]; then
        [ -n "$pidfile_proc" ] && {
            if [ "$(uname)" = "Darwin" ]; then
                ...
                #  still around
                #
                kill -0 "$pidfile_proc" 2>/dev/null && b_was_alive=true
            else
                [ -d /proc/"$pidfile_proc" ] && b_was_alive=true
            fi
            $b_was_alive && {
...
pidfile_acquire() {
    ...
    [ -n "$pid_file" ] && [ ! -f "$pid_file" ] && {
        ...
        #  or running out of _pa_attempts.
        #
        echo $$ 2>/dev/null >"$pid_file" || {
            _pa_msg="pidfile_acquire($pid_file) \n"
            _pa_msg="$_pa_msg Early grab failed to write pid_file"
            error_msg "$_pa_msg" -1
            return 1
        }
    ...
    ! is_int "$_pa_attempts" && {
        _pa_msg="pidfile_acquire($pid_file) \n"
        _pa_msg="$_pa_msg 2nd param must be int - got [$_pa_attempts]"
        error_msg "$_pa_msg"
    }
    ...
    while [ "$_pa_i" -le "$_pa_attempts" ]; do
        ...
        if ! pidfile_is_live "$pid_file"; then
            echo "$$" 2>/dev/null >"$pid_file" || {
                _pa_msg="pidfile_acquire($pid_file) \n "
                _pa_msg="${_pa_msg}Failed to write pid_file"
                error_msg "$_pa_msg" -1
                return 2
            }
            _pf_log "pid_file created"

            log_it "+++++ $pid_file_short available on attempt $_pa_i/$_pa_attempts"
            break
        fi
    ...
    #  Final verification
    pidfile_is_mine "$pid_file" || {
        _pa_msg="pidfile_acquire($pid_file) - Failed \n"
        _pa_msg="$_pa_msg it is used by process: $pidfile_proc"
        error_msg "$_pa_msg:" -1
        return 3
    }
    _pf_log "Acquire successful"
...
pidfile_release() {
    ...
    pidfile_is_live "$pid_file" && {
        pidfile_is_mine "$pid_file" || {
            _pr_msg="pidfile_release($pid_file) failed \n"
            _pr_msg="$_pr_msg in use by process: $pidfile_proc"
            error_msg "$_pr_msg" -1
            return 1
        }
    }
